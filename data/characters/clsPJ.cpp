...
void pj::addGravity(float f){
    _velocity.y+=f;//3.0f;
 }

void pj::isGravity(pj pj,sf::RectangleShape obj){

    if(pj.getHitbox().getGlobalBounds().intersects(obj.getGlobalBounds())){
            setGravity(true);
      }
      else setGravity(false);
}

void pj::setGravity(bool e){
    gravity=e;
}
