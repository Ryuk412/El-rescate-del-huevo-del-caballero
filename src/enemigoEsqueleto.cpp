...
void enemigoEsqueleto::danioRecibido(int danio){esq_vida=esq_vida-danio;}
...
void enemigoEsqueleto::update(int limitA, int limitB){
    ...
    {
        _frame+=0.009;
        esq_sprite.setTextureRect({0 + (int)_frame*130,0,130,130});
        if(_frame>=4)
        {
            _frame=0;
        }
    ...
    if(esq_velocity.x==0&&esq_velocity.y==0 ){
        ...
        esq_sprite.setTextureRect({0 + (int)_frame*130,130,130,130});
...
sf::RectangleShape enemigoEsqueleto::getHitbox()const{return esq_hitbox;}
