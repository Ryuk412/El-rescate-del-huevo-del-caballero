...
Jefe::Jefe()
{
    ...
    j_hitbox.setSize({20,10});
    j_hitbox.setFillColor(sf::Color::Magenta);
    j_sprite.setPosition(300,230);
    j_vida=500;
...
if(j_velocity.x==0&& j_velocity.y==0)
    {
        _frame+=0.050;
        j_sprite.setTextureRect({0 + (int)_frame*115,0,130,130}); //Ancho y Alto del Personaje,70x65.
        if(_frame>=4)
        {
            _frame=0;
        }
    ...
    if(j_velocity.x==0&&j_velocity.y==0 )
    {
        ...
        j_sprite.setTextureRect({130 + (int)_frame*115,130,130,130}); //Ancho y Alto del Personaje,70x65.
        if(_frame>=6)
        {
            _frame=0;
        }
