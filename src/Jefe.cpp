...
if (j_hitbox.getPosition().x <= LimiteA) {
    _direccion = 1; // Cambiar dirección a la derecha
}
else if (j_hitbox.getPosition().x + j_hitbox.getSize().x >= LimiteB) {
    _direccion = -1; // Cambiar dirección a la izquierda
}
